"""
CCSlideCreatorAgent（スライド作成役 - Claude Code版）
...
class CCSlideCreatorAgent(ClaudeCodeBaseAgent):
    """Claude Codeを使用してレポートからスライドを作成するエージェント"""
    ...
    def __init__(self, background_image: str = "images/slideBackground.png"):
        super().__init__(
            name="CCSlideCreatorAgent",
            description="スライド作成役 - レポートの内容をスライドにする（Claude Code版）"
        )
...
"""
    ...
    def generate_marp_header(self) -> List[str]:
        ...
        if not bg_path.startswith('../') and not bg_path.startswith('/'):
            bg_path = f"../{bg_path}"
        content.append(f"    background-image: url('{bg_path}');")
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            実行結果
                ...
                - slides_data: スライドデータ
        """
        self.log("Starting slide creation with Claude Code...")
        ...
        if not slides_data:
            ...
            self.log("Generating slide structure from report with Claude Code...")
