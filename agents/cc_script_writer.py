"""
CCScriptWriterAgent（脚本家 - Claude Code版）
スライドから動画のシナリオを考えるエージェント
Gemini APIの代わりにClaude Code CLIを使用
"""
...
class CCScriptWriterAgent(ClaudeCodeBaseAgent):
    """Claude Codeを使用してスライドから動画脚本を作成するエージェント"""

    def __init__(self):
        super().__init__(
            name="CCScriptWriterAgent",
            description="脚本家 - スライドから動画のシナリオを考える（Claude Code版）"
        )
...
"""
    ...
    def generate_full_script(self, slides: List[Dict]) -> List[Dict]:
        ...
        for i, slide in enumerate(slides):
            ...
            # 連続リクエストの間に少し待つ（Claude Code CLIの負荷軽減）
            if i < len(slides) - 1:
                time.sleep(2)

        return scripts

    def generate_all_scripts_at_once(self, slides: List[Dict]) -> List[Dict]:
        """
        すべてのスライドの原稿を一度に生成（効率的なバージョン）
        ...
        for slide in slides:
            slides_text += f"""
---
スライド {slide['index']}: {slide['title']}
{slide['content']}
"""

        prompt = f"""
あなたはプレゼンテーションの原稿を作成する専門家です。
以下の全スライドに対して、それぞれ自然で分かりやすい原稿を日本語で作成してください。

## スライド一覧
{slides_text}

## 要件
1. 各スライドに対して、自然な話し言葉で原稿を作成
2. 1スライドあたり30〜60秒程度の長さ
3. 箇条書きは自然な文章に変換
4. 聞き手に語りかけるような口調
5. 「えー」「あのー」などは入れない
6. スライド番号や「このスライドでは」などの言及は避ける

## 出力フォーマット
各スライドの原稿を以下の形式で出力してください：
...
"""
        ...
        for match in matches:
            ...
            script_text = self.clean_declarative_phrases(match[1].strip())

            # 対応するスライドを探す
            for slide in slides:
                if slide['index'] == slide_num:
                    scripts.append({
                        'index': slide_num,
                        'title': slide['title'],
                        'script': script_text
        ...
        # マッチしなかったスライドがあれば個別に生成
        if len(scripts) < len(slides):
            self.log("Some scripts not parsed, generating individually...", "WARNING")
            matched_indices = {s['index'] for s in scripts}
            for slide in slides:
                if slide['index'] not in matched_indices:
                    script = self.generate_script_for_slide(slide, len(slides))
                    scripts.append({
                        'index': slide['index'],
                        'title': slide['title'],
                        'script': script
        ...
        # インデックス順にソート
        scripts.sort(key=lambda x: x['index'])
        return scripts
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            context: 実行コンテキスト
                ...
                - batch_mode: True の場合、全スライドを一度に処理（デフォルト: True）
        ...
        """
        self.log("Starting script writing with Claude Code...")
        ...
        if script_notes:
            ...
            if batch_mode and len(slides) <= 15:
                self.log("Using batch mode for script generation...")
                scripts = self.generate_all_scripts_at_once(slides)
            else:
