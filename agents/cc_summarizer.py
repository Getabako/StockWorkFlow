"""
CCSummarizerAgent（まとめ役 - Claude Code版）
...
class CCSummarizerAgent(ClaudeCodeBaseAgent):
    """Claude Codeを使用して情報をまとめてレポートを作成するエージェント"""

    def __init__(self):
        super().__init__(
            name="CCSummarizerAgent",
            description="まとめ役 - 日々の情勢をまとめ、レポートに仕立て上げる（Claude Code版）"
        )
...
---
*このレポートはAI（Claude）により自動生成されています。投資判断は必ずご自身で行ってください。*
"""
        return prompt
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        self.log("Starting summarization with Claude Code...")
...
---
*このレポートはAI（Claude）により自動生成されています。*
"""
        else:
            # プロンプトを生成
            ...
            # Claude Codeで分析
            self.log(f"Analyzing {len(articles)} articles with Claude Code...")
